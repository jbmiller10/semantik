...
async def process_document(document: Document, collection: Collection, session: Any) -> bool:
    ...
    try:
        ...
        # Create chunks
        chunker = TokenChunker(chunk_size=collection.chunk_size or 1000, chunk_overlap=collection.chunk_overlap or 200)
        ...
        for i in range(0, len(points), batch_size):
            ...
            upsert_request = {"collection_name": collection.vector_store_name, "points": points_data, "wait": True}
...
async def process_collection(collection_name_or_id: str) -> None:
    ...
    async with AsyncSessionLocal() as session:
        ...
        stmt = select(Collection).where(
            (Collection.name == collection_name_or_id) | (Collection.id == collection_name_or_id)
        )
