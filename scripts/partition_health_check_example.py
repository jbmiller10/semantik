    ...
    try:
        ...
        response = {
            ...
            "performance": {
                ...
                "avg_chunks_per_partition": round(metrics["avg_chunks_per_partition"], 1) if metrics["partition_count"] > 0 else 0,
...
async def verify_partition_keys(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to verify partition keys: {str(e)}"
        )
...
async def periodic_partition_health_check(db: AsyncSession) -> None:
    ...
    try:
        ...
        if metrics["hot_partitions"]:
            logger.warning(
                f"Detected {len(metrics['hot_partitions'])} hot partitions "
                f"with uneven data distribution"
            )
