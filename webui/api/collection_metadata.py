        ...
        if not any(c.name == METADATA_COLLECTION for c in collections):
            qdrant.create_collection(
                ...
                vectors_config=VectorParams(
                    size=4,  # Small vector size, we're not using it for search
...
def store_collection_metadata(qdrant: QdrantClient, collection_name: str, 
                            model_name: str, quantization: str,
                            vector_dim: int, chunk_size: int = None,
                            chunk_overlap: int = None, instruction: str = None):
