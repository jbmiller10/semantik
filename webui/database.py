...
def create_job(job_data: Dict[str, Any]) -> str:
    ...
    c.execute('''INSERT INTO jobs 
                 ...
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
              (job_data['id'], job_data['name'], job_data['description'],
               job_data['status'], job_data['created_at'], job_data['updated_at'],
               job_data['directory_path'], job_data['model_name'],
               job_data['chunk_size'], job_data['chunk_overlap'],
               job_data['batch_size'], job_data.get('vector_dim'),
...
def update_file_status(job_id: str, file_path: str, status: str, error: Optional[str] = None,
                      chunks_created: int = 0, vectors_created: int = 0):
