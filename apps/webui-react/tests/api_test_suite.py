...
class APITestSuite:
    ...
    async def test_job_creation(self) -> Optional[str]:
        """Test job creation"""
    ...
    async def test_directory_scan(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            try:
                async with session.post(
                    f"{self.base_url}/api/scan-directory",
                    headers=self.headers,
                    json=scan_data
                ) as resp:
                    if resp.status == 200:
                        ...
                        self.log_test("Directory Scan", True, f"Found {file_count} files", 
            ...
            try:
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        self.log_test("Vector Search", True, f"Found {len(results)} results", 
                    ...
                    async with session.post(
                        ...
                        if resp.status == 200:
                            ...
                            if search_mode == "hybrid":
                                ...
                                self.log_test("Hybrid Search", True, "Hybrid search is available", 
        ...
        async with aiohttp.ClientSession() as session:
            ...
            try:
                async with session.post(
                    ...
                    if resp.status == 200:
                        data = await resp.json()
                        self.log_test("Search Filters", True, "Filters accepted", 
    ...
    async def test_search_performance(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            try:
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        self.log_test("Search Performance", elapsed < 1000, 
            ...
            try:
                ...
                self.log_test("Concurrent Search", success_count == 5,
                            f"{success_count}/5 successful in {elapsed:.2f}ms")
            except Exception as e:
    ...
    async def test_document_preview(self):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(
                    f"{self.base_url}/api/documents/preview",
                    headers=self.headers,
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(
                    ...
                    if resp.status == 200:
                        data = await resp.json()
                        self.log_test("Settings", True, "Settings retrieved", 
    ...
    async def test_error_handling(self):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(
                    f"{self.base_url}/api/jobs/invalid-job-id-12345",
                    headers=self.headers
                ) as resp:
                    if resp.status in [404, 400]:
                        self.log_test("Error Handling - Invalid Job", True, 
                                    f"Properly handled with HTTP {resp.status}")
                    else:
                        self.log_test("Error Handling - Invalid Job", False, 
        ...
        try:
            async with session.post(
                ...
                if resp.status in [400, 422]:
                    self.log_test("Error Handling - Invalid Search", True,
                                f"Properly handled with HTTP {resp.status}")
                else:
    ...
    def print_summary(self):
        ...
        features = {
            ...
            "WebSocket Directory Scan": any(r["test"] == "Directory Scan WebSocket" and r["success"] for r in self.test_results),
