...
class AdaptiveBatchSizeManager:
    ...
    def __init__(self, default_safety_margin: float = 0.2):
        ...
        logger.info(f"Initialized AdaptiveBatchSizeManager with safety margin: {default_safety_margin}")
    ...
    def get_current_batch_size(self, model_name: str, quantization: str = "float32") -> int | None:
        """
    ...
    def update_batch_size(self, model_name: str, quantization: str, new_size: int) -> None:
        """
        ...
        logger.info(f"Updated batch size for {model_name}/{quantization}: {old_size} -> {new_size}")

    def reset_batch_size(self, model_name: str, quantization: str) -> None:
        """
        ...
        with self._lock:
            if key in self._batch_sizes:
                ...
                logger.info(f"Reset batch size for {model_name}/{quantization} (was: {old_size})")
            else:
                logger.debug(f"No batch size to reset for {model_name}/{quantization}")
    ...
    def get_or_calculate_batch_size(
        ...
        initial_size = self.calculate_initial_batch_size(model_name, quantization, text_length, safety_margin)
