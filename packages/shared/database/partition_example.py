...
class ChunkingServiceExample:
    ...
    async def update_chunk_embeddings_example(
        ...
        chunk_updates = [
            ...
            for chunk_id, embedding_id in zip(chunk_ids, embeddings)
        ]
    ...
    async def search_chunks_example(
        self,
        collection_id: str,
        ...
        if document_id:
            chunks = await self.chunk_repo.get_chunks_by_document(
                document_id=document_id,
                collection_id=collection_id,  # Always include!
                limit=10
            )
    ...
    async def cleanup_chunks_example(
        self,
        collection_id: str,
        document_id: str
    ) -> None:
...
async def main():
    ...
    async with AsyncSessionLocal() as session:
        ...
        # 3. Query chunks
        await service.search_chunks_example(
            collection_id=collection_id,
            document_id=document_id
        )
        
        # 4. Cleanup
        await service.cleanup_chunks_example(
            collection_id=collection_id,
            document_id=document_id
        )
