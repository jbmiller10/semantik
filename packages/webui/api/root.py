...
@router.get("/pipeline/{conversation_id}")
async def pipeline_builder_page(conversation_id: str) -> FileResponse:  # noqa: ARG001
    """Serve the React app for pipeline builder route"""
    base_dir = Path(__file__).resolve().parent.parent
    return FileResponse(base_dir / "static" / "index.html")
