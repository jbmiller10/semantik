...
class ProjectionService:
    ...
    def _encode_projection(
        ...
        meta_payload: dict[str, Any] | None = meta if meta else None
    ...
    async def select_projection_region(
        ...
        for selected_id in ordered_ids:
            id_index = id_to_index.get(selected_id)
            if id_index is None:
                missing_ids.append(selected_id)
                continue

            original_identifier: str | None = None
            if original_ids and 0 <= id_index < len(original_ids):
                raw_identifier = original_ids[id_index]
