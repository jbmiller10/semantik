...
from packages.webui.core.celery_app import celery_app
from packages.webui.core.redis import get_redis_client
...
chunking_memory_usage = Gauge(
    "chunking_memory_usage_bytes",
    "Current memory usage by chunking tasks",
    ["operation_id"],
)
...
def process_chunking_operation(
    ...
    try:
        ...
        loop.run_until_complete(
            _handle_soft_timeout(operation_id, correlation_id, self)
        )
    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            ...
            await operation_repo.update_status(
                ...
                {"task_id": task_id, "started_at": datetime.utcnow().isoformat()},
            )
            ...
            # Track metrics
            chunking_task_duration.labels(operation_type="chunking").observe(
            ...
            logger.info(
                ...
                extra={
                    "operation_id": operation_id,
                    "cleanup_result": cleanup_result.to_dict() if hasattr(cleanup_result, "to_dict") else str(cleanup_result),
                },
            )
            ...
            chunking_memory_usage.labels(operation_id=operation_id).set(0)
...
async def _monitor_resources(
    ...
    chunking_memory_usage.labels(operation_id=operation_id).set(current_memory)
...
def monitor_dead_letter_queue() -> dict[str, Any]:
    ...
    try:
        ...
        if dlq_size > 0:
            ...
            failed_tasks = [
                json.loads(task)
                for task in redis_client.lrange(dlq_key, 0, 9)
            ]
