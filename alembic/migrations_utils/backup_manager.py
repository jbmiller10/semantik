        ...
        with self.Session() as session:
            try:
                ...
                count_result = session.execute(
                    text(f"SELECT COUNT(*) FROM {backup_table_name}")
                )
                ...
                if actual_count != expected_count:
                    logger.error(
                        f"Count mismatch for {backup_table_name}: "
                        f"expected {expected_count}, found {actual_count}"
                    )
                    return False
                
                logger.info(
                    f"Backup {backup_table_name} verified: {actual_count} records"
                )
                return True
    ...
    def extend_retention(self, backup_table_name: str, days: int = 7) -> bool:
        ...
        with self.Session() as session:
            try:
                ...
                session.commit()
                logger.info(
                    f"Extended retention for {backup_table_name} by {days} days"
                )
                return True
...
def main():
    ...
    parser.add_argument(
        "--action",
        choices=["list", "cleanup", "verify", "extend", "status"],
        required=True,
        help="Action to perform"
    )
    parser.add_argument(
        "--table-name",
        help="Backup table name (for verify/extend actions)"
    )
    parser.add_argument(
        "--days",
        type=int,
        default=7,
        help="Days to extend retention (for extend action)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Perform dry run (for cleanup action)"
    )
