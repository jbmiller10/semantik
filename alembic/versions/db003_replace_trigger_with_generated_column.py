...
def upgrade() -> None:
    ...
    if not impl_status["has_trigger"]:
        logger.warning("No trigger found. This might be a fresh installation. Setting up GENERATED column directly.")
    ...
    if chunk_count > 0 and not verify_partition_keys(conn):
        raise ValueError(
            "Existing partition keys have inconsistencies. Please fix these before converting to GENERATED column."
        )
...
def downgrade() -> None:
    ...
    if impl_status["is_partitioned"]:
        ...
        logger.info("=" * 60)
        logger.info("The chunks table is partitioned. Ensuring trigger-based implementation is in place.")
