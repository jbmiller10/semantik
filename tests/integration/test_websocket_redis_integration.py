...
class TestWebSocketRedisIntegration:
    ...
    async def test_end_to_end_job_updates_flow(self, real_redis_mock, mock_websocket_factory):
        ...
        with patch("webui.websocket_manager.redis.from_url", return_value=real_redis_mock):
            ...
            with patch("webui.websocket_manager.create_job_repository") as mock_create_repo:
                mock_repo = AsyncMock()
                mock_repo.get_job = AsyncMock(return_value={
                    "status": "processing",
                    "total_files": 10,
                    "processed_files": 0
                })
                ...
                await celery_updater.send_update("progress", {
                    "progress": 25,
                ...
                progress_updates = [
                    msg for msg in ws_client.received_messages 
    ...
    async def test_multiple_clients_receive_updates(self, real_redis_mock, mock_websocket_factory):
        ...
        with patch("webui.websocket_manager.redis.from_url", return_value=real_redis_mock):
            ...
            clients = [
                mock_websocket_factory(f"client{i}") for i in range(3)
            ]
            ...
            with patch("webui.websocket_manager.create_job_repository") as mock_create_repo:
                ...
                await manager.send_job_update("job1", "broadcast", {
                    "message": "Update for all clients"
                })
    ...
    async def test_message_history_replay(self, real_redis_mock, mock_websocket_factory):
        ...
        with patch("webui.websocket_manager.redis.from_url", return_value=real_redis_mock):
            ...
            with patch("webui.websocket_manager.create_job_repository") as mock_create_repo:
                ...
                update_messages = [
                    msg for msg in client.received_messages 
                    if msg["type"] in ["update1", "update2", "update3"]
                ]
    ...
    async def test_consumer_group_coordination(self, real_redis_mock, mock_websocket_factory):
        ...
        with patch("webui.websocket_manager.redis.from_url", return_value=real_redis_mock):
            ...
            with patch("webui.websocket_manager.create_job_repository") as mock_create_repo:
                ...
                await manager1.send_job_update("job1", "shared_update", {
                    "message": "Update from manager1"
                })
                ...
                client1_updates = [
                    msg for msg in client1.received_messages 
                    if msg["type"] == "shared_update"
                ]
                client2_updates = [
    ...
    async def test_stream_cleanup_after_job_completion(self, real_redis_mock):
        ...
        with patch("webui.websocket_manager.redis.from_url", return_value=real_redis_mock):
            ...
            assert f"job:updates:job1" in real_redis_mock.streams
            
            # Clean up the stream
            await manager.cleanup_job_stream("job1")
            
            # Verify stream is deleted
            assert f"job:updates:job1" not in real_redis_mock.streams
