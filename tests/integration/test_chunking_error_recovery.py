...
class TestChunkingOperationInterruption:
    ...
    async def test_graceful_shutdown_handling(
        ...
        with patch("webui.chunking_tasks.ChunkingTask._graceful_shutdown", True):
            ...
            with patch.object(task, "_save_progress") as mock_save:
                # Simulate partial processing
                try:
        ...
        chunks_before = await async_session.execute(
...
class TestCircuitBreaker:
    ...
    async def test_circuit_breaker_activation(
        ...
        with patch("webui.chunking_tasks.ChunkingTask._increment_failure_count") as mock_increment:
            with patch("webui.chunking_tasks.ChunkingTask._check_circuit_breaker") as mock_check:
    ...
    async def test_circuit_breaker_recovery(
        ...
        with patch("webui.chunking_tasks.ChunkingTask._check_circuit_breaker") as mock_check:
            with patch("webui.chunking_tasks.time.time") as mock_time:
