...
def calculate_partition_key(collection_id: str, document_id: str = None) -> int:
    ...
    """
    key_string = f"{collection_id}:{document_id}" if document_id else f"{collection_id}:null"
...
class TestPartitionDistribution:
    ...
    async def test_collection_locality(
        ...
        result = await async_session.execute(
            select(Chunk.partition_key).where(Chunk.collection_id == collection.id).distinct()
        )
...
class TestPartitionQueryPerformance:
    ...
    async def test_partition_pruning_effectiveness(
        ...
        # Warm up the query
        await async_session.execute(select(Chunk).where(Chunk.collection_id == collection.id).limit(1))

        # Measure query with partition pruning (includes collection_id)
        start_time = time.time()
        result_with_pruning = await async_session.execute(select(Chunk).where(Chunk.collection_id == collection.id))
        ...
        start_time = time.time()
        result_without_pruning = await async_session.execute(select(Chunk).where(Chunk.document_id == doc_id))
    ...
    async def test_cross_partition_query_performance(
        ...
        start_time = time.time()
        result = await async_session.execute(select(Chunk).where(Chunk.collection_id.in_(collection_ids)))
...
class TestPartitionMaintenance:
    ...
    async def test_identify_hot_partitions(
        ...
        for _ in range(10):
            await async_session.execute(select(Chunk).where(Chunk.collection_id == hot_collection.id).limit(10))
        ...
        result = await async_session.execute(
            ...
            .where(Chunk.collection_id == hot_collection.id)
            .group_by(Chunk.partition_key)
        )
        ...
        for _partition_key, count in hot_partitions:
            assert count > 0
...
class TestPartitionFailureScenarios:
    ...
    async def test_partition_rebalancing_simulation(
        ...
        overloaded_partitions = [p for p, count in original_distribution.items() if count > threshold]
