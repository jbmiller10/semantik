...
class TestFixedExample:
    ...
    async def test_basic_chunking_with_fixtures(
        ...
        result = await async_session.execute(select(Chunk).where(Chunk.collection_id == collection.id))
    ...
    async def test_edge_cases_with_special_documents(
        ...
        _chunks_created = await ChunkingTestHelper.simulate_chunking_process(
            session=async_session,
            redis_client=redis_client,
            operation_id=operation_id,
            collection_id=collection.id,
