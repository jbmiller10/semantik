...
class TestChunkRepository:
    ...
    async def test_create_chunks_bulk_auto_generate_ids(self, repository, mock_session):
        ...
        def capture_bulk_insert(func):
            sync_session = Mock()
            sync_session.bulk_insert_mappings = Mock(
                side_effect=lambda model, items: inserted_data.extend(items)
            )
            func(sync_session)
        ...
        assert all("id" in chunk for chunk in inserted_data)
    ...
    async def test_get_chunk_by_id_found(self, repository, mock_session, sample_chunk_data):
        ...
        result = await repository.get_chunk_by_id(
            sample_chunk_data["id"],
            sample_chunk_data["collection_id"]
        )
    ...
    async def test_get_chunk_by_id_not_found(self, repository, mock_session):
        ...
        result = await repository.get_chunk_by_id(str(uuid4()), str(uuid4()))
    ...
    async def test_get_chunks_by_document_with_pagination(self, repository, mock_session):
        ...
        await repository.get_chunks_by_document(
            str(uuid4()), str(uuid4()), limit=10, offset=20
        )
    ...
    async def test_get_chunks_by_collection(self, repository, mock_session):
        ...
        result = await repository.get_chunks_by_collection(
            collection_id, limit=10, created_after=created_after
        )
    ...
    async def test_update_chunk_embeddings_invalid_data(self, repository):
        ...
        with pytest.raises(ValueError, match="must have 'id', 'collection_id', and 'embedding_vector_id'"):
            await repository.update_chunk_embeddings([{"id": str(uuid4())}])
    ...
    async def test_update_chunk_embeddings_batch_size_limit(self, repository):
        ...
        chunk_updates = [
            ...
            for _ in range(10001)  # Exceeds MAX_BATCH_SIZE
        ]
    ...
    async def test_get_chunk_statistics(self, repository, mock_session):
        ...
        with patch(
            "packages.shared.database.repositories.chunk_repository.ChunkPartitionHelper"
        ) as mock_helper:
            mock_stats = {
                "collection_id": collection_id,
                "chunk_count": 50,
            ...
            mock_helper.get_partition_statistics.assert_called_once_with(
                mock_session, collection_id
            )
