...
""",
            "very_long": " ".join([f"Sentence number {i} contains unique information about topic {i % 5}." for i in range(1000)]),
        }
    ...
    def test_performance_monitoring(self, mock_embed_model, sample_texts):
        ...
        with patch('time.time') as mock_time:
            ...
            with patch('packages.shared.text_processing.chunking_metrics.performance_monitor.measure_chunking') as mock_monitor:
    ...
    def test_chunk_with_splitter_helper(self, mock_embed_model, sample_texts):
        ...
        chunks = chunker._chunk_with_splitter(
            mock_splitter,
            sample_texts["simple"],
            "helper_test",
            metadata
    ...
    async def test_async_chunk_with_metadata_override(self, mock_embed_model):
        ...
        chunks = await chunker.chunk_text_async(
            "Test text for metadata override",
            "metadata_override_doc",
            original_metadata
        )
