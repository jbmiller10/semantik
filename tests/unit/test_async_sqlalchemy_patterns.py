        ...
        for py_file in repository_dir.glob("*.py"):
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Call):
                    # Check if it's a method call on session
                    if (isinstance(node.func, ast.Attribute) and 
                        node.func.attr == "delete" and
                        isinstance(node.func.value, ast.Attribute) and
                        node.func.value.attr == "session"):
    ...
    def test_delete_imports_in_repositories(self):
        ...
        for py_file in repository_dir.glob("*.py"):
            ...
            if "async def delete" in content:
                assert "from sqlalchemy import" in content and "delete" in content, \
                    f"{py_file.name} has a delete method but doesn't import delete from sqlalchemy"
                ...
                for i, line in enumerate(lines):
                    if "async def delete_" in line:
                        ...
                        if "await self.collection_repo.delete" in method_body:
                            assert "await self.db_session.commit()" in method_body, \
    ...
    def test_flush_vs_commit_pattern(self):
        ...
        for py_file in repository_dir.glob("*.py"):
            ...
            if "self.session.commit()" in content:
                assert False, f"{py_file.name} uses commit() instead of flush() - repositories should use flush"
        ...
        for py_file in services_dir.glob("*.py"):
            ...
            if "await self." in content and "_repo." in content and any(op in content for op in ["create", "update", "delete"]):
                # Check if it has db_session and uses commit
                if "self.db_session" in content and "commit()" not in content:
                    ...
                    if any(f"async def {op}" in content for op in write_ops):
                        assert False, f"{py_file.name} has write operations but doesn't commit transactions"
        ...
        for py_file in packages_dir.rglob("*.py"):
            ...
            for i, line in enumerate(lines):
                ...
                if "# type: ignore" in line and ("coroutine" in line or "unused-coroutine" in line):
                    violations.append({
                        "file": py_file.relative_to(packages_dir),
                        "line": i + 1,
                        "code": line.strip()
                    })
