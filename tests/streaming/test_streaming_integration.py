...
The streaming processor should handle this efficiently."""
        ...
        try:
            ...
            processor = StreamingDocumentProcessor(checkpoint_manager=checkpoint_manager, memory_pool=memory_pool)
            strategy = SimpleChunkingStrategy()

            # Process document
            chunks = []
            async for chunk in processor.process_document(temp_file, strategy, mock_config):
                chunks.append(chunk)
    ...
    async def test_pipeline_concurrent_processing(self, mock_config) -> None:
        ...
        try:
            ...
            async def process_file(file_path: str, file_id: int) -> tuple[int, int]:
                ...
                chunks = []
                async for chunk in processor.process_document(file_path, strategy, mock_config):
                    chunks.append(chunk)
