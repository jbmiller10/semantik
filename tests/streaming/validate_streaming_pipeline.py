...
async def validate_memory_usage() -> None:
    ...
    try:
        ...
        async for _ in processor.process_document(test_file, strategy, config):
            chunks_processed += 1
        ...
        # Results
        print("\nResults:")
        print("  File size: 100MB (simulating 10GB)")
        print(f"  Chunks processed: {chunks_processed}")
        print(f"  Max memory used: {max_memory_used:.2f}MB")
        print(
            f"  Average memory: {sum(memory_samples)/len(memory_samples):.2f}MB" if memory_samples else "  No samples"
        )

        if max_memory_used < 100:
            print("✅ PASSED: Memory stayed under 100MB limit")
            return True

        print("❌ FAILED: Memory exceeded 100MB limit")
        return False
    ...
    try:
        ...
        async for chunk in processor.process_document(test_file, strategy, config):
            ...
            try:
                ...
                _ = encoded.decode("utf-8", errors="strict")
        ...
        if not corruption_found:
            print("✅ PASSED: No UTF-8 corruption detected")
            return True

        print("❌ FAILED: UTF-8 corruption detected")
        return False
...
async def validate_checkpoint_resume() -> None:
    ...
    try:
        ...
        try:
            async for chunk in processor1.process_document(test_file, strategy1, config, operation_id=operation_id):
        ...
        print(f"  Checkpoint saved: {checkpoint.chunks_processed} chunks, {checkpoint.byte_position} bytes")
        ...
        chunks_phase2 = []
        async for chunk in processor2.process_document(test_file, strategy2, config, operation_id=operation_id):
        ...
        print("✅ PASSED: Checkpoint/resume working correctly")
        return True
...
async def validate_progress_events() -> None:
    ...
    try:
        ...
        chunks = []
        async for chunk in processor.process_document(test_file, strategy, config, progress_callback=progress_callback):
            chunks.append(chunk)
        ...
        if progress_events:
            ...
            if intervals:
                ...
                if max_interval < 2.0:  # Allow up to 2 seconds between events
                    print("✅ PASSED: Progress events emitted regularly")
                    return True

                print("❌ FAILED: Progress events too infrequent")
                return False

        print("❌ FAILED: No progress events received")
        return False
...
async def validate_backpressure() -> None:
    ...
    try:
        ...
        if stats["in_use"] == 0:
            print("✅ PASSED: Backpressure managed memory correctly")
            return True

        print("❌ FAILED: Memory not properly released")
        return False
...
async def validate_edge_cases() -> None:
    ...
    config.estimate_chunks = lambda t: 1  # noqa: ARG005
