...
class TestRelevanceScoring:
    ...
    def test_compute_relevance_scores_basic(self, mock_softmax, mock_stack, reranker_loaded, 
                                           mock_transformers, sample_documents, mock_model_output):
    ...
    @patch('torch.stack')
    @patch('torch.nn.functional.softmax')
    def test_compute_relevance_scores_empty_documents(self, mock_softmax, mock_stack, reranker_loaded, 
                                                     mock_transformers, mock_model_output):
        ...
        mock_tokenizer.return_value = MagicMock(
            input_ids=torch.randint(0, 1000, (len(documents), 10))
        )
...
class TestEdgeCases:
    ...
    @patch('torch.stack')
    @patch('torch.nn.functional.softmax')
    def test_very_long_documents(self, mock_softmax, mock_stack, reranker_loaded, 
                                mock_transformers, mock_model_output):
    ...
    @patch('torch.stack')
    @patch('torch.nn.functional.softmax')
    def test_special_token_encoding_fallback(self, mock_softmax, mock_stack, reranker_loaded, 
                                            mock_transformers, mock_model_output):
