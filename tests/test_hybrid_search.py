...
def test_search_api_hybrid():
    ...
    # Test queries
    test_queries = [
        {"query": "machine learning algorithms", "mode": "filter", "keyword_mode": "any"},
        {"query": "docker container deployment", "mode": "rerank", "keyword_mode": "all"},
        {"query": "python data analysis", "mode": "filter", "keyword_mode": "any", "score_threshold": 0.5},
    ]
    ...
    for test in test_queries:
        ...
        try:
            # Make request
            params = {"q": test["query"], "k": 5, "mode": test["mode"], "keyword_mode": test["keyword_mode"]}
...
def test_keyword_search():
    ...
    test_queries = [
        {"query": "python docker kubernetes", "mode": "any"},
        {"query": "machine learning model", "mode": "all"},
    ]
    ...
    for test in test_queries:
        ...
        try:
            params = {"q": test["query"], "k": 5, "mode": test["mode"]}
...
def test_direct_hybrid_search():
    ...
    try:
        ...
        results = hybrid_engine.hybrid_search(
            query_vector=query_vector, query_text=query, limit=3, hybrid_mode="filter"
        )
        ...
        print("\nRerank mode results:")
        results = hybrid_engine.hybrid_search(
            query_vector=query_vector, query_text=query, limit=3, hybrid_mode="rerank"
        )
...
def check_api_health():
    ...
    apis = [("Search API", f"{SEARCH_API_URL}/"), ("WebUI API", f"{WEBUI_API_URL}/api/health")]
