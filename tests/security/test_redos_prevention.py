...
from packages.shared.chunking.utils.safe_regex import SafeRegex, RegexTimeoutError
...
class TestRegexSafetyModule:
    ...
    @pytest.mark.skipif(
        True, reason="Actual timeout test may be flaky in CI"
    )
...
class TestSafeRegexClass:
    ...
    def test_pattern_caching(self, safe_regex):
        ...
        pattern1 = safe_regex.compile_safe(r"\d+")
        pattern2 = safe_regex.compile_safe(r"\d+")
...
class TestChunkingValidation:
    ...
    def test_validate_content_with_safe_text(self):
        ...
        ChunkingInputValidator.validate_content(
            "This is safe content with no injection patterns.",
    ...
    def test_validate_content_with_bounded_patterns(self):
        ...
        try:
            ...
            pass  # We don't care if it's rejected, just that it doesn't hang
