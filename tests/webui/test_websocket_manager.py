...
class TestRedisStreamWebSocketManager:
    ...
    async def test_connect_connection_limit(self, manager, mock_websocket):
        ...
        mock_websocket.close.assert_called_once_with(
            code=1008, reason="Connection limit exceeded"
        )
    ...
    async def test_consume_updates(self, manager, mock_redis, mock_websocket):
        ...
        mock_redis.xreadgroup.return_value = [
            ("job:updates:job1", [
                ("msg-id-1", {"message": json.dumps(test_message)})
            ])
        ]
    ...
    async def test_send_history(self, manager, mock_redis, mock_websocket):
        ...
        historical_messages = [
            ("msg-1", {"message": json.dumps({
                "timestamp": "2024-01-01T00:00:00",
                "type": "start",
                "data": {"status": "started"}
            })}),
            ("msg-2", {"message": json.dumps({
    ...
    async def test_concurrent_connections(self, manager, mock_redis):
        ...
        with patch("webui.websocket_manager.create_job_repository") as mock_create_repo:
            ...
            connect_tasks = [
                manager.connect(ws, "job1", f"user{i}")
        ...
        total_connections = sum(
            len(sockets) for sockets in manager.connections.values()
        )
