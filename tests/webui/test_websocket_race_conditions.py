...
async def test_throttling_race_condition(manager):
    ...
    async def check_throttle(check_id):
        ...
        result = await manager._should_send_progress_update(operation_id, message)
        return result
...
async def test_connection_limit_enforcement_race_condition(manager):
    ...
    accepted = sum(1 for r in results 
                  if not isinstance(r, Exception) and r.accepted)
...
async def test_stress_test_all_operations(manager):
    ...
    for op_id in manager._chunking_progress_throttle.keys():
        # Should only have recent entries, not from cleaned operations
        assert "op_user" not in op_id or datetime.now(UTC).timestamp() - \
               manager._chunking_progress_throttle[op_id].timestamp() < 10