...
class TestChunkingMetrics:
    ...
    async def test_metrics_recorded_when_chunking_succeeds_with_strategy(self, service):
        ...
        chunks_count = self.get_counter_value(
            ingestion_chunks_total, 
            {"strategy": result["stats"]["strategy_used"]}
        )
    ...
    async def test_fallback_metrics_invalid_config(self, service, mock_token_chunker):
        ...
        with patch.object(service.config_builder, 'build_config') as mock_build:
            mock_build.return_value = MagicMock(
                validation_errors=["Invalid parameter: invalid_param"],
                strategy="semantic",
                config={}
            )
        ...
        fallback_count = self.get_counter_value(
            ingestion_chunking_fallback_total,
            {"strategy": "semantic", "reason": "invalid_config"}
        )
    ...
    async def test_fallback_metrics_runtime_error(self, service, mock_token_chunker):
        ...
        fallback_count = self.get_counter_value(
            ingestion_chunking_fallback_total,
            {"strategy": "markdown", "reason": "runtime_error"}
        )
        assert fallback_count == 1
    ...
    async def test_fallback_metrics_config_error(self, service, mock_token_chunker):
        ...
        fallback_count = self.get_counter_value(
            ingestion_chunking_fallback_total,
            {"strategy": "hierarchical", "reason": "config_error"}
        )
        assert fallback_count == 1
    ...
    async def test_duration_histogram_records_reasonable_values(self, service):
        ...
        for strategy_name in ["character", "ChunkingStrategy.CHARACTER"]:
            count = self.get_histogram_count(
                ingestion_chunking_duration_seconds,
        ...
        histogram_count = self.get_histogram_count(
            ingestion_chunking_duration_seconds,
            {"strategy": strategy_used}
        )
        histogram_sum = self.get_histogram_sum(
            ingestion_chunking_duration_seconds,
            {"strategy": strategy_used}
        )
    ...
    async def test_chunk_count_metrics_increment_correctly(self, service):
        ...
        # Verify total chunks
        total_chunks = self.get_counter_value(
            ingestion_chunks_total,
            {"strategy": strategy_used}
        )
    ...
    async def test_multiple_operations_accumulate_metrics_correctly(self, service, mock_token_chunker):
        ...
        token_chunks = self.get_counter_value(
            ingestion_chunks_total,
            {"strategy": "TokenChunker"}
        )
        ...
        for strategy_name in ["character", "ChunkingStrategy.CHARACTER"]:
            count = self.get_counter_value(
                ingestion_chunking_fallback_total,
                {"strategy": strategy_name, "reason": "runtime_error"}
            )
            if count > 0:
    ...
    def test_helper_functions_work_correctly(self):
        ...
        sum_val = self.get_histogram_sum(
            ingestion_chunking_duration_seconds,
            {"strategy": "test_strategy"}
        )
        assert count == 1
        ...
        fallback_count = self.get_counter_value(
            ingestion_chunking_fallback_total,
            {"strategy": "original_strategy", "reason": "test_reason"}
        )
        assert fallback_count == 1
