...
class TestDirectoryScanService:
    ...
    async def test_scan_directory_preview_success(
        ...
        counting_calls = [
            call for call in mock_ws_manager._broadcast.call_args_list
    ...
    def test_should_include_file(
        ...
        assert directory_scan_service._should_include_file(
            Path("test.pdf"), None, None
        ) is True
        
        # Test unsupported extension
        assert directory_scan_service._should_include_file(
            Path("test.jpg"), None, None
        ) is False
        
        # Test include patterns
        assert directory_scan_service._should_include_file(
            Path("test.pdf"), ["*.pdf"], None
        ) is True
        assert directory_scan_service._should_include_file(
            Path("test.txt"), ["*.pdf"], None
        ) is False
        
        # Test exclude patterns
        assert directory_scan_service._should_include_file(
            Path("test.pdf"), None, ["*.pdf"]
        ) is False
        assert directory_scan_service._should_include_file(
            Path("test.txt"), None, ["*.pdf"]
        ) is True
        
        # Test both include and exclude patterns
        assert directory_scan_service._should_include_file(
            Path("test.pdf"), ["*.pdf", "*.txt"], ["test.*"]
        ) is False  # Excluded takes precedence
    ...
    def test_get_mime_type(
        ...
        # Test Word documents
        assert directory_scan_service._get_mime_type(Path("test.docx")) == \
            "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
        assert directory_scan_service._get_mime_type(Path("test.doc")) == "application/msword"
        
        # Test PowerPoint
        assert directory_scan_service._get_mime_type(Path("test.pptx")) == \
    ...
    async def test_progress_updates_during_scan(
        ...
        final_progress = next(
            (call for call in reversed(progress_calls) if call[0][1]["data"].get("percentage") == 100.0),
            None
        )
