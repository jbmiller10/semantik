...
class TestLargeDocumentIngestion:
    ...
    async def test_append_large_document_with_segmentation(
        ...
        with patch('packages.webui.tasks.get_async_session_context') as mock_session_ctx:
            with patch('packages.webui.tasks.CollectionRepository') as MockCollectionRepo:
                with patch('packages.webui.tasks.DocumentRepository') as MockDocumentRepo:
                    with patch('packages.webui.tasks.OperationRepository') as MockOperationRepo:
                    with patch('packages.webui.tasks.create_celery_chunking_service_with_repos') as mock_create_service:
                        with patch('packages.webui.tasks.extract_text') as mock_extract:
                            with patch('packages.webui.tasks.embed_and_upsert') as mock_embed:
                                ...
                                MockOperationRepo.return_value = mock_op_repo
                ...
                with patch('packages.webui.tasks.DocumentRepository') as MockDocumentRepo:
                    with patch('packages.webui.tasks.OperationRepository') as MockOperationRepo:
                        with patch('packages.webui.tasks.create_celery_chunking_service_with_repos') as mock_create_service:
                            with patch('packages.webui.tasks.extract_text') as mock_extract:
                                with patch('packages.webui.tasks.embed_and_upsert') as mock_embed:
                                    with patch('packages.webui.tasks.QdrantManager') as MockQdrantManager:
                                        ...
                                        MockQdrantManager.return_value = mock_qdrant
    ...
    async def test_strategy_specific_segmentation_thresholds(self):
        ...
        from packages.webui.services.chunking_service import ChunkingService
        from packages.webui.services.chunking_constants import STRATEGY_SEGMENT_THRESHOLDS
    ...
    async def test_segmentation_error_recovery(self):
        ...
        async def mock_process_with_failures(*args, **kwargs):
            ...
            if segment_count % 3 == 0:
                raise Exception(f"Segment {segment_count} failed")
            
            return {
                "chunks": [
                    {"chunk_id": f"chunk_{segment_count:04d}", "text": f"chunk {segment_count}", "metadata": {}}
                ]
            }
